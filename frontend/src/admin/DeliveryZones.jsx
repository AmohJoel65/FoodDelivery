  ...
  if (loading) {
    return (
      <div className="py-20 flex flex-col justify-center items-center text-left">
        ...
        <p className="text-[10px] font-bold tracking-widest text-[#1a1a1a]/50 uppercase mt-4">Loading Delivery Zones...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in select-none text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#ebdcae]/10 pb-6">
        <div>
          <h1 className="text-3xl font-bold font-serif text-[#1a1a1a]">Delivery Zones</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {zones.length === 0 ? (
          <div className="col-span-full bg-white border border-[#ebdcae]/15 rounded-3xl p-12 text-center">
            <MapPin size={48} className="mx-auto text-[#1a1a1a]/20 mb-4" />
            <p className="text-sm text-[#1a1a1a]/40 font-light italic">No delivery zones configured yet.</p>
          </div>
        ) : (
          zones.map((zone) => (
            <div key={zone._id} className="bg-white border border-[#ebdcae]/15 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
              ...
              <h3 className="text-lg font-bold text-[#1a1a1a] mb-2">{zone.name}</h3>
              <p className="text-2xl font-bold text-[#d4af37] mb-3">{zone.baseFee} FCFA</p>

              <div className="space-y-2 text-xs mb-4">
                <div className="flex justify-between">
                  <span className="text-[#1a1a1a]/50">Distance:</span>
                  <span className="font-semibold text-[#1a1a1a]">{zone.distanceKm} km</span>
                </div>
              </div>

              <div className="flex flex-wrap gap-1.5 mb-4">
                {zone.areas.slice(0, 3).map((area, idx) => (
                  <span key={idx} className="px-2 py-1 bg-[#1a1a1a]/5 text-[#1a1a1a]/60 text-[10px] rounded-full">
                    {area}
                  </span>
                ))}
                {zone.areas.length > 3 && (
                  <span className="px-2 py-1 bg-[#1a1a1a]/5 text-[#1a1a1a]/60 text-[10px] rounded-full">
              ...
              {zone.description && (
                <p className="text-xs text-[#1a1a1a]/50 line-clamp-2">{zone.description}</p>
              )}
            </div>
          ))
        )}
        ...
        <div className="fixed inset-0 bg-[#1a1a1a]/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold tracking-widest uppercase text-[#1a1a1a]/60 mb-2">Zone Name</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div>
                <label className="block text-[10px] font-bold tracking-widest uppercase text-[#1a1a1a]/60 mb-2">Areas (comma-separated)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold tracking-widest uppercase text-[#1a1a1a]/60 mb-2">Base Fee (FCFA)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-[10px] font-bold tracking-widest uppercase text-[#1a1a1a]/60 mb-2">Distance (km)</label>
                <input
                  type="number"
